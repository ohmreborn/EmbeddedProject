...
const uint8_t title_w = (uint8_t)title_len * 11; // Font_11x18 width approx
const uint8_t title_x = (uint8_t)((128 - title_w) / 2);
const uint8_t title_y = ui_margin + 2; // a bit spacing above title

const char scroll_text[] = "Press button to start";
const uint8_t btn_w = 80;
const uint8_t btn_h = 14;
const uint8_t x1 = (128 - btn_w) / 2;
const uint8_t y1 = 64 - ui_margin - btn_h; // keep bottom margin equal to top margin
const uint8_t x2 = x1 + btn_w;
const uint8_t y2 = y1 + btn_h;
...
const uint8_t txt_y = y1 + (btn_h > txt_h ? (btn_h - txt_h) / 2 : 0);
