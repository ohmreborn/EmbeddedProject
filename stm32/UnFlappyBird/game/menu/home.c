...
static int16_t scroll_pos = 128;
static char scroll_text[] = "Press button to start";
static const uint8_t scroll_speed = 2;
...
void Home_Update(void){
  if (HAL_GPIO_ReadPin(START_BTN_PORT, START_BTN_PIN) == GPIO_PIN_SET){
...
void Home_Render(void){
  ssd1306_Fill(Black);
    ...
    int16_t title_w = (int16_t)title_len * 11; // Font_11x18 width approx
    ...
    size_t tlen = strlen(scroll_text);
    int16_t text_width = (int16_t)tlen * 6; // Font_6x8 approx width per char = 6
    const int16_t spacing = 12; // gap between repetitions
    int16_t full_width = text_width + spacing;
    scroll_pos -= scroll_speed;
    if (scroll_pos <= -full_width) {
      scroll_pos += full_width; // wrap but keep continuity
    }
    int16_t scroll_y = title_y + 18 + 4;
    // draw primary
    ssd1306_SetCursor(scroll_pos, scroll_y);
    ssd1306_WriteString(scroll_text, Font_6x8, White);
    // draw second copy following the first
    ssd1306_SetCursor(scroll_pos + full_width, scroll_y);
    ssd1306_WriteString(scroll_text, Font_6x8, White);
